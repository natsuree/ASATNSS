        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="asat-card">
                <div class="divide-y divide-gray-100">
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h3 class="font-extrabold text-slate-900">{{ $notification->title }}</h3>
                                ...
                                @unless ($notification->is_read)
                                    <form method="POST" action="{{ route('notifications.update', $notification) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-sm font-bold text-[var(--asat-navy)] hover:text-[var(--asat-teal)]">
                                            {{ __('Mark as read') }}
                                        </button>
                                    </form>
                                @endunless
                    ...
                    @empty
                        <div class="p-6 text-sm text-slate-600">{{ __('No notifications yet.') }}</div>
                    @endforelse
                </div>
